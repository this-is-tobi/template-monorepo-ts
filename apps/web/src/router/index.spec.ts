...
describe('router', () => {
  ...
  it('should define all expected routes', async () => {
    ...
    expect(routeNames).toContain('account-profile')
    expect(routeNames).toContain('account-security')
  ...
  it('should mark app pages as requiresAuth', async () => {
    ...
    const profile = router.getRoutes().find(r => r.name === 'account-profile')
    const security = router.getRoutes().find(r => r.name === 'account-security')
    ...
    expect(security?.meta.requiresAuth).toBe(true)
  })
