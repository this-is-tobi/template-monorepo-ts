...
export const testRoutes = [
  ...
  { path: '/organizations/:id/roles', name: 'organization-roles', component: Stub, meta: { requiresAuth: true } },
  ...
  { path: '/profile', name: 'profile', component: Stub, meta: { requiresAuth: true } },
  {
    ...
    children: [
      ...
      { path: 'api-keys', name: 'settings-api-keys', component: Stub, meta: { requiresAuth: true, requiresAdmin: true } },
    ],
  },
...
export const primevueStubs = {
  ...
  DataTable: { template: '<div><slot v-if="!value || value.length === 0" name="empty" /><slot /></div>', props: ['value', 'stripedRows', 'tableStyle'] },
