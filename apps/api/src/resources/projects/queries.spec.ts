...
describe('[Projects] - Queries', () => {
  ...
  describe('getProjectsQuery', () => {
    it('should get all projects when no filters provided', async () => {
      const full = mockProject(data)
      dbRo.project.findMany.mockResolvedValueOnce([full])
  ...
  describe('getProjectByIdQuery', () => {
    it('should get project by its ID', async () => {
      const full = mockProject(data)
      dbRo.project.findUnique.mockResolvedValueOnce(full)

      const project = await getProjectByIdQuery(data.id)

      expect(dbRo.project.findUnique).toHaveBeenCalledTimes(1)
      expect(dbRo.project.findUnique).toHaveBeenCalledWith({ where: { id: data.id } })
      expect(project).toStrictEqual(full)
    })
  })
  ...
  describe('getProjectDetailQuery', () => {
    it('should get project with owner data', async () => {
      ...
      expect(dbRo.project.findUnique).toHaveBeenCalledTimes(1)
      expect(dbRo.project.findUnique).toHaveBeenCalledWith({
        where: { id: data.id },
  ...
  describe('getProjectMemberQuery', () => {
    it('should find a member by projectId and userId', async () => {
      ...
      expect(dbRo.projectMember.findUnique).toHaveBeenCalledWith({
  ...
  describe('getProjectMemberByIdQuery', () => {
    it('should find a member by id', async () => {
      ...
      expect(dbRo.projectMember.findUnique).toHaveBeenCalledWith({ where: { id: memberId } })
      expect(result).toStrictEqual(member)
    })
  })
  ...
  describe('getProjectIdsForUser', () => {
    it('should return project IDs for a user', async () => {
      ...
      expect(dbRo.projectMember.findMany).toHaveBeenCalledWith({
        where: { userId },
  ...
  describe('countProjects', () => {
    it('should count all projects when no filters provided', async () => {
      dbRo.project.count.mockResolvedValueOnce(5)

      const count = await countProjects()

      expect(dbRo.project.count).toHaveBeenCalledWith(undefined)
      expect(count).toBe(5)
    })

    it('should count projects with filters', async () => {
      dbRo.project.count.mockResolvedValueOnce(2)

      const count = await countProjects({ ownerId: data.ownerId })

      expect(dbRo.project.count).toHaveBeenCalledWith({ where: { ownerId: data.ownerId } })
      expect(count).toBe(2)
    })
  })
  ...
  describe('countUserOrganizations', () => {
    it('should count organizations a user belongs to', async () => {
      dbRo.member.count.mockResolvedValueOnce(3)

      const count = await countUserOrganizations(data.ownerId)

      expect(dbRo.member.count).toHaveBeenCalledWith({ where: { userId: data.ownerId } })
      expect(count).toBe(3)
    })
  })
  ...
  describe('countProjectsInOrganization', () => {
    it('should count projects in an organization', async () => {
      const orgId = randomUUID()
      dbRo.project.count.mockResolvedValueOnce(7)

      const count = await countProjectsInOrganization(orgId)

      expect(dbRo.project.count).toHaveBeenCalledWith({ where: { organizationId: orgId } })
      expect(count).toBe(7)
    })
  })
  ...
  describe('getProjectsQuery with accessibleBy', () => {
    it('should add OR conditions when accessibleBy is set', async () => {
      ...
      dbRo.projectMember.findMany.mockResolvedValueOnce([{ projectId: memberProjectId } as never])
      // getOrgIdsWithProjectAccess — user is owner in this org
      dbRo.member.findMany.mockResolvedValueOnce([{ organizationId: orgId, role: 'owner' } as never])
      dbRo.project.findMany.mockResolvedValueOnce([])

      await getProjectsQuery({ accessibleBy: userId })

      expect(dbRo.project.findMany).toHaveBeenCalledWith({
        where: {
          OR: [
            { ownerId: userId },
    ...
    it('should exclude orgs where user is only a member (no project:read)', async () => {
      ...
      dbRo.member.findMany.mockResolvedValueOnce([{ organizationId: randomUUID(), role: 'member' } as never])
      dbRo.project.findMany.mockResolvedValueOnce([])
  ...
  describe('getOrgMaxProjects', () => {
    it('should return null when organization has no metadata', async () => {
      dbRo.organization.findUnique.mockResolvedValueOnce({ metadata: null } as never)
    ...
    it('should return maxProjects from metadata', async () => {
      dbRo.organization.findUnique.mockResolvedValueOnce({ metadata: JSON.stringify({ maxProjects: 5 }) } as never)
    ...
    it('should return 0 when maxProjects is explicitly 0', async () => {
      dbRo.organization.findUnique.mockResolvedValueOnce({ metadata: JSON.stringify({ maxProjects: 0 }) } as never)
    ...
    it('should return null when metadata has no maxProjects key', async () => {
      dbRo.organization.findUnique.mockResolvedValueOnce({ metadata: JSON.stringify({ personal: true }) } as never)
    ...
    it('should return null when metadata is invalid JSON', async () => {
      dbRo.organization.findUnique.mockResolvedValueOnce({ metadata: 'not-json' } as never)
  ...
  describe('getUserByEmailQuery', () => {
    it('should find a user by email', async () => {
      const userId = randomUUID()
      dbRo.user.findFirst.mockResolvedValueOnce({ id: userId } as never)
  ...
  describe('getOrgIdsWithProjectAccess', () => {
    ...
    it('should include orgs via custom role with project:read', async () => {
      ...
      dbRo.organizationRole.findMany.mockResolvedValueOnce([
  ...
  describe('isPersonalOrg', () => {
    ...
    it('should return false for a non-personal organization', async () => {
      dbRo.organization.findUnique.mockResolvedValueOnce({ metadata: JSON.stringify({ personal: false }) } as never)
  ...
  describe('getUserByIdQuery', () => {
    it('should find a user by ID', async () => {
      ...
      expect(dbRo.user.findUnique).toHaveBeenCalledWith({ where: { id: userId }, select: { id: true } })
      expect(result).toStrictEqual({ id: userId })
    })
  })
  ...
  describe('isOrgMember', () => {
    it('should return true when user is a member', async () => {
      dbRo.member.count.mockResolvedValueOnce(1)

      const result = await isOrgMember('u-1', 'org-1')

      expect(dbRo.member.count).toHaveBeenCalledWith({ where: { userId: 'u-1', organizationId: 'org-1' } })
      expect(result).toBe(true)
    })

    it('should return false when user is not a member', async () => {
      dbRo.member.count.mockResolvedValueOnce(0)
