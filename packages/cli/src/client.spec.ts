...
describe('createClient', () => {
  ...
  it('creates client with correct base URL', async () => {
    const config: ResolvedConfig = { serverUrl: 'http://localhost:3000', output: 'table' }
  ...
  it('sets bearer token header when token is provided', async () => {
    const config: ResolvedConfig = { serverUrl: 'http://localhost:3000', token: 'my-token', output: 'table' }
    const client = createClient(config)
    await client.system.getVersion()
  ...
  it('sets API key header when apiKey is provided', async () => {
    const config: ResolvedConfig = { serverUrl: 'http://localhost:3000', apiKey: 'my-key', output: 'table' }
    const client = createClient(config)
    await client.system.getVersion()
